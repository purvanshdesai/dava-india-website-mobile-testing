...
export default function LastMinBuy() {
...
//   )
  ...
  const associatedProducts = products ?? []
      ...
      <div className='z-10'>
        <Carousel opts={{ slidesToScroll: 'auto' }}>
          <div className='flex items-center justify-between'>
            <p className='text-sm font-semibold'>{cart('last_minute_buy')}</p>
          ...
          <CarouselContent className='gap-0'>
            {associatedProducts &&
              associatedProducts.map((product: any, index: number) => (
                <CarouselItem
                  key={index}
                  className='fade-in-up animate-delay-100 relative my-2 flex basis-2/3 items-center justify-center px-2.5 opacity-0'
